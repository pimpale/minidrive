...
use vulkano::sync::future::{SubmitAnyBuilder, AccessCheckError};
...
#[inline]
pub fn now(queue: Arc<Queue>) -> QueuedNowFuture {
    QueuedNowFuture { queue }
}

/// A dummy future that represents "now".
pub struct QueuedNowFuture {
    queue: Arc<Queue>,
}

unsafe impl GpuFuture for QueuedNowFuture {
    #[inline]
...
unsafe impl DeviceOwned for QueuedNowFuture {
    #[inline]
